...
// This function takes a list of lists as input and returns the length of a list that contains all the unique words from all the input lists.
function lengthOfAllLists(lists) {

  // If the input list is empty or if the first element of the input list is a string, then simply return the length of the input list.
  if (lists.length && typeof lists[0] == 'string') {
    return lists.length;
  }

  // Create a new empty list to store the unique words from all the input lists.
  let new_list = [];

  // Iterate over the input list and merge the current element of the input list with the new list.
  lists.forEach(function(a) {
    new_list = combineLists(new_list, a);
  });

  // Remove any duplicate words from the new list.
